...
namespace kobuki_msgs
{
  ...
  class DigitalOutput : public ros::Msg
  {
    ...
    {
      int offset = 0;
      unsigned char * values_val = (unsigned char *) this->values;
      ...
      *(outbuffer + offset + 0) = (u_valuesi.base >> (8 * 0)) & 0xFF;
      ...
      unsigned char * mask_val = (unsigned char *) this->mask;
      for( uint8_t i = 0; i < 4; i++){
      union {
        bool real;
        uint8_t base;
    ...
    {
      int offset = 0;
      uint8_t * values_val = (uint8_t*) this->values;
      for( uint8_t i = 0; i < 4; i++){
      union {
        bool real;
        uint8_t base;
      ...
      uint8_t * mask_val = (uint8_t*) this->mask;
      for( uint8_t i = 0; i < 4; i++){
      union {
        bool real;
        uint8_t base;

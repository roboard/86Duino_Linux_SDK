/*
  ...
  Copyright (c) 2016 Android Lin <acen@dmp.com.tw>. All right reserved.
...
Servo::Servo() {
	if (ServoCount < MAX_SERVOS)
	{
		...
		{
			OSSPININIT(servo86); 
			// setpriority(PRIO_PROCESS, 0, -20); // increase the priority of servo86 lib.
		}
...
uint8_t Servo::attach(int pin, unsigned long min, unsigned long max) {
	...
	if (this->servoIndex < MAX_SERVOS)
	{
		...
		this->min = min;
		this->max = max; 
		if (isPWMPin(pin) == false) return 0;
		
		// Only use hardware PWM pin
...
void Servo::run(void) {
	...
	{
		Timer1.initialize();
		Timer1.attachInterrupt(isr_handler, 5000L); // updating period = 5ms
	    Timer_Used = true;
	}
}
...
void Servo::pause(void) {
	...
	if (sv86[servoIndex].state != SERVO_MOVING)
	{
		#if defined (DMP_DOS_DJGPP) || defined (DMP_DOS_BC)
			io_RestoreINT();
		#else
...
void Servo::resume(void) {
    ...
    if (sv86[servoIndex].state != SERVO_PAUSE)
	{
		#if defined (DMP_DOS_DJGPP) || defined (DMP_DOS_BC)
			io_RestoreINT();
		#else
